...
import handling.world.World;
 ...
 public class CWvsContext
 {
   ...
   public static byte[] charInfo(MapleCharacter chr, boolean isSelf) {
     ...
     for (Iterator i$ = prof.iterator(); i$.hasNext(); ) { int i = ((Integer)i$.next()).intValue();
       mplew.writeShort(i);
     }
     if (chr.getGuildId() <= 0) {
     ...
     for (Iterator i$ = chairs.iterator(); i$.hasNext(); ) { int i = ((Integer)i$.next()).intValue();
       mplew.writeInt(i);
     }
   ...
   private static byte[] serverMessage(int type, int channel, String message, boolean megaEar) {
     ...
     case 19:
     case 21:
     case 23: } return mplew.getPacket();
   }
  ...
  public static byte[] pendantSlot(boolean p) { //slot -59
   ...
   public static class FamilyPacket
   {
     ...
     public static byte[] getFamilyInfo(MapleCharacter chr) {
       ...
       for (Iterator i$ = b.iterator(); i$.hasNext(); ) { int ii = ((Integer)i$.next()).intValue();
         mplew.writeInt(ii);
         mplew.writeInt(1);
       }
     ...
     public static byte[] getFamilyPedigree(MapleCharacter chr) {
       ...
       int descendants = 2; int gens = 0; int generations = 0;
       if (family == null) {
         mplew.writeInt(2);
  ...
  public static byte[] giveKilling(int x) {
   ...
   public static class PartyPacket
   {
     ...
     public static byte[] updateParty(int forChannel, MapleParty party, PartyOperation op, MaplePartyCharacter target) {
      ...
      switch (op) {
         ...
         if (op == PartyOperation.DISBAND) break;
         mplew.write(op == PartyOperation.EXPEL ? 1 : 0);
         mplew.writeMapleAsciiString(target.getName());
         addPartyStatus(forChannel, party, mplew, op == PartyOperation.LEAVE); break;
         case JOIN:
         mplew.write(19);
         mplew.writeInt(party.getId());
         mplew.writeMapleAsciiString(target.getName());
   ...
   public static class InfoPacket
   {
     ...
     public static byte[] updateQuestMobKills(MapleQuestStatus status) {
       ...
       for (Iterator i$ = status.getMobKills().values().iterator(); i$.hasNext(); ) { int kills = ((Integer)i$.next()).intValue();
   ...
   public static class BuffPacket
   {
    ...
    public static byte[] giveForeignPirate(Map<MapleBuffStat, Integer> statups, int duration, int cid, int skillid) {
        ...
        public static byte[] giveArcane(Map<Integer, Integer> statups, int localDuration) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
       // mplew.write(HexTool.getByteArrayFromHexString("00 00 00 00 00 00 00 00 40 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 04 00 8B 66 23 00 88 13 00 00 00 00 00 00 00 00"));
        mplew.writeShort(SendPacketOpcode.GIVE_BUFF.getValue());
     ...
     public static byte[] giveBuff(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect) {
       ...
       for (Map.Entry stat : statups.entrySet()) {
         ...
         mplew.writeInt(bufflength);
                ...
                if (buffid == 24121004) { // phantom aria buff :D
                  mplew.writeZeroBytes(3);
                  mplew.writeShort(0); // not sure..
                  mplew.write(0);
                 }
