...
public class PlayerHandler
{
  ...
  public static final void ChangeKeymap(LittleEndianAccessor slea, MapleCharacter chr){
    if ((slea.available() > 8L) && (chr != null)){
      ...
      int type = slea.readInt(); int data = slea.readInt();
      switch (type){
      case 1:
        if (data <= 0)
  ...
  public static final void TakeDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr){
      ...
      if (defType == 1){
       ...
       if(GameConstants.isDualBlade(chr.getJob())){
         if(damage > 0){
             ...
             int bof = chr.getTotalSkillLevel(ads);
        if (bof > 0){
          MapleStatEffect eff = ads.getEffect(bof);
          if (Randomizer.nextInt(100) <= eff.getX()){
            SkillFactory.getSkill(Rogue.DARK_SIGHT).getEffect(1).applyTo(chr);
          }
        }
                }
       if(damage < 1){
          Skill shadow_meld = SkillFactory.getSkill(DualBlade.SHADOW_MELD);
         int bof = chr.getTotalSkillLevel(shadow_meld);
          if (bof > 0){
              SkillFactory.getSkill(DualBlade.SHADOW_MELD).getEffect(1).applyTo(chr);
         }
        }
       }
      if (skillid != 0){
    ...
    if (damage > 0){
      ...
      if ((chr.inPVP()) && (chr.getStat().getHPPercent() <= 20)){
        chr.getStat(); SkillFactory.getSkill(PlayerStats.getSkillByJob(93, chr.getJob())).getEffect(1).applyTo(chr);
      }
    }
    byte offset = 0;
    int offset_d = 0;
  ...
  public static final void rangedAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr){
    ...
    int bulletCount = 1; int skillLevel = 0;
    MapleStatEffect effect = null;
    Skill skill = null;
    ...
    int projectile = 0; int visProjectile = 0;
    ...
    if ((res != null) && (c.getPlayer().getMap() != null)){
      ...
      if ((samepos) && ((pos.y > map.getBottom() + 250) || (map.getFootholds().findBelow(pos) == null))){
        if (count > 5){
          ...
          count++; c.getPlayer().setFallCounter(count);
        }
      } else if (count > 0){
        c.getPlayer().setFallCounter(0);
      }
  ...
  public static final void ChangeMap(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr){
    ...
    if (slea.available() != 0L){
      ...
      if ((targetid != -1) && (!chr.isAlive())){
        ...
        boolean unlock = false; boolean warp = false;
        if (divi == 9130401){
