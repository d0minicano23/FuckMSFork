...
public class MapleAndroid implements Serializable {
	...
	if (!changed){
        ...
        try {
            final PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("UPDATE androids SET name = ?, hair = ?, face = ? WHERE uniqueid = ?");
            ps.setString(1, name);
            ps.setInt(2, hair);
            ps.setInt(3, face);
    ...
    public static final MapleAndroid create(int itemid, int uniqueid, int hair, int face){
        ...
        try { // Commit to db first
            PreparedStatement pse = DatabaseConnection.getConnection().prepareStatement("INSERT INTO androids (uniqueid, name, hair, face) VALUES (?, ?, ?, ?)");
